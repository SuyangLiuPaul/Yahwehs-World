...
import { BLOCK_M, MODELS, stamp, type Model } from './creatures.ts';
...
for (const name of names) {
  ...
  const across = turn % 2 ? m.width : m.length;
  ...
  const originX = turn === 0 ? centre - (m.x0 + m.length / 2)
    : turn === 2 ? centre + (m.x0 + m.length / 2)
