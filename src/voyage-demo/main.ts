...
// A standalone demo, not yet linked from the site. The globe's own routes
// show these same models at map scale — a few pixels across, correctly, at
// the distance a whole itinerary is read from. This page asks a different
// question: brought up to a scale where the text no longer names anything,
// how much can these generated pieces (D19, handoff/13-asset-library.md)
// actually carry — real water, a lit shore, an outline that reads as a
// character rather than a specimen? Nothing here claims to be an excavated
// place; it never puts a name to what it shows, which is exactly why it is
// allowed the licence a labelled map is not.
...
// A touch brighter and more saturated than the tabernacle's 1.05 — this scene
// is asking to read as bright and cheerful, not as reverent interior light.
renderer.toneMappingExposure = 1.22;
renderer.shadowMap.enabled = true;
renderer.shadowMap.type = THREE.PCFSoftShadowMap;

const scene = new THREE.Scene();
const camera = new THREE.PerspectiveCamera(42, innerWidth / innerHeight, 0.1, 2500);
...
// and simulations have a failure mode a hand-authored gradient does not: its
// Rayleigh term away from the sun runs weak enough, even at a generous
// turbidity, that an orbiting camera drifts into near-black sky the moment it
// swings past the anti-solar point — found by isolating the shader from the
// rest of the scene and sweeping the sun-to-view angle, not by guessing.
// A cheerful, stylised scene never wanted the physics; it wanted a sky that
// is never wrong to look at, so this is a plain two-stop gradient instead —
// simpler, and closer to what "cute" actually asks for than atmospheric
// scattering was going to be anyway.
...
scene.environment = pmrem.fromScene(skyEnvScene, 0, 0.1, 2200).texture;
...
scene.add(sunLight);
scene.add(new THREE.HemisphereLight(0xbcdcff, 0xc9a877, 0.55));
scene.fog = new THREE.FogExp2(0xcfe6ea, 0.0016);

// ── water ────────────────────────────────────────────────────────────────
// A generated tileable normal map — an analytic sum of five periodic swells,
// each an integer number of cycles across the tile so it wraps with no seam —
// rather than a downloaded water-normals photo. Encodes a real slope field,
// not decorative noise: the highlights the Water shader draws from it are the
// slope of an actual (if invented) wave surface, not a texture that merely
// looks wet.
...
const water = new Water(new THREE.PlaneGeometry(4000, 4000), {
  ...
  waterNormals: oceanNormalMap(512),
...
// geometry carries the shape, the generated ground/rock studies from Phase 3
// carry the surface detail.
...
// "Bigger and cuter, no problem" — explicit direction this round, and the
// opposite instinct from the globe (where these figures are meant to be
// found, not stared at). CUTE_SCALE only pushes them past life-size; it does
// not touch proportion, which stays whatever the source model already is.
...
(async () => {
  const PERSON_HEIGHT = 1.72; // metres, a plain human-scale reference — this
  // scene has no procedural miniature to fit against, so it fits to a person.
  ...
  // Ship: fit to a plausible real length for a coastal grain vessel (source
  // gives no measurement — Acts 27 names the ship, not its size — so this is
  // a chosen, stated number, not a recovered one) and lift onto the water.
  ...
  // exists in the animation library actually searched for one). Parented to
  // `shipGroup`, not `shipGltf.scene`: the loaded scene carries the hull's own
  // scale-up to real length, and a child placed inside it would inherit that
  // scale a second time.
  ...
  const mixers: THREE.AnimationMixer[] = [sailor.mixer];
  ...
  // ── HUD wiring ───────────────────────────────────────────────────────
  let animating = true, outlinesOn = true;
  document.getElementById('toggle-anim')!.addEventListener('click', (e) => {
    animating = !animating;
    (e.currentTarget as HTMLButtonElement).setAttribute('aria-pressed', String(animating));
    (e.currentTarget as HTMLButtonElement).textContent = animating ? '暂停动作' : '继续动作';
  });
  document.getElementById('toggle-outline')!.addEventListener('click', (e) => {
    outlinesOn = !outlinesOn;
    scene.traverse((o) => { if (o.renderOrder === -1) o.visible = outlinesOn; });
    (e.currentTarget as HTMLButtonElement).setAttribute('aria-pressed', String(outlinesOn));
    (e.currentTarget as HTMLButtonElement).textContent = '卡通轮廓：' + (outlinesOn ? '开' : '关');
  });
  document.getElementById('toggle-orbit')!.addEventListener('click', (e) => {
    controls.autoRotate = !controls.autoRotate;
    (e.currentTarget as HTMLButtonElement).setAttribute('aria-pressed', String(controls.autoRotate));
    (e.currentTarget as HTMLButtonElement).textContent = '自动环绕：' + (controls.autoRotate ? '开' : '关');
  });
  ...
  if (import.meta.env.DEV) {
    (globalThis as unknown as Record<string, unknown>).__voyage = {
      scene, camera, renderer, controls, water, shipGroup, walkers, mixers,
